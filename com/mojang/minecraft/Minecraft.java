...
public class Minecraft implements Runnable {
   ...
   public void tick() {
      while(Mouse.next()) {
         if (this.currentScreen != null) {
            if (Mouse.getEventButtonState()) {
               int sw = this.width * 240 / this.height;
               int sh = this.height * 240 / this.height;
               int mx = Mouse.getEventX() * sw / this.width;
               int my = sh - Mouse.getEventY() * sh / this.height - 1;
               this.currentScreen.mouseClicked(mx, my, Mouse.getEventButton());
            }
            continue;
         }
      ...
      while(true) {
         ...
         if (this.currentScreen != null) {
            this.currentScreen.keyPressed(Keyboard.getEventCharacter(), Keyboard.getEventKey());
            continue;
         }
   ...
   private void setupCamera(float a) {
      GL11.glMatrixMode(5889);
      GL11.glLoadIdentity();
      GLU.gluPerspective((float)this.settings.fov, (float)this.width / (float)this.height, 0.05F, 1000.0F);
      GL11.glMatrixMode(5888);
      GL11.glLoadIdentity();
   ...
   public void render(float a) {
      ...
      if (this.mouseGrabbed) {
         ...
         float sens = (float)this.settings.mouseSensitivity / 5.0F;
         this.player.turn(xo * sens, yo * (float)this.yMouseAxis * sens);
      }
      ...
      if (this.currentScreen != null) {
         int sw = this.width * 240 / this.height;
         int sh = this.height * 240 / this.height;
         int mx = Mouse.getX() * sw / this.width;
         int my = sh - Mouse.getY() * sh / this.height - 1;
         this.currentScreen.render(mx, my);
      }
   ...
   private void setupFog(int i) {
      float density = this.settings.getFogDistance();
      if (i == 0) {
         ...
         GL11.glFogi(2917, 2048);
         GL11.glFogf(2914, density * 10.0F);
